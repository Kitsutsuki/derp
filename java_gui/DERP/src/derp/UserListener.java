...
public class UserListener implements ListSelectionListener {
    ...
    public UserListener(JTable table, SocketIO socket) {
	this.socket = socket;
	this.table = table;
    }
	...
	panel = (UserPanel) table.getParent().getParent().getParent().getParent();
	try {
	    switch (table.getSelectedRow()) {
		case 0:
		    value = JOptionPane.showInputDialog(null, "Nom d'utilisateur :", table.getValueAt(table.getSelectedRow(), 1));
		    if (value != null) {
			socket.emit("message", User.update(panel.getSelectedId(), "login", value));
		    }
		    break;
		case 1:
		    ...
		    if (value != null) {
			socket.emit("message", User.update(panel.getSelectedId(), "status", value));
		    }
		    break;
		case 2:
			...
			if (value.equals(conf)) {
			    socket.emit("message", User.update(panel.getSelectedId(), "pass", value));
			} else {
